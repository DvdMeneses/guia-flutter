...
import 'package:guia_project/providers/motel_provider.dart'; // Importe o MotelProvider
...
void main() {
  group('fetchMoteis', () {
    test(
      "return moteis é feito com sucesso",
      () async {
        final client = MockClient();
        final motelProvider = Moteis();
        when(client.get(Uri.parse('https://www.jsonkeeper.com/b/1IXK')))
            .thenAnswer((_) async => http.Response(json, 200));
        // Chama a função fetchMotelData e verifica o resultado
        await motelProvider.fetchMotelData();

        expect(motelProvider.moteis[0].fantasia, 'Motel Le Nid');
    ...
    test(
      ...
      () async {
        final client = MockClient();
        final motelProvider = Moteis(client: client);
        ...
        verify(client.get(Uri.parse('https://www.jsonkeeper.com/b/1IXK')));
      },
    );
  });
}
